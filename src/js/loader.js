...
Promise.all([
  ...
  Object.keys(trainTimetableRefData).forEach(function(key) {
    trainTimetableRefData[key]
      .filter(function(table) {
        return (
          table.r === 'JR-East.Keiyo' &&
          (table.tt[0].ds === 'JR-East.Keiyo.NishiFunabashi' || table.tt[table.tt.length - 1].as === 'JR-East.Keiyo.NishiFunabashi')
        );
      })
      .forEach(function(table) {
        ...
        var branchName =
          (startFromNishiFunabashi && direction === 'Outbound') || (!startFromNishiFunabashi && direction === 'Inbound')
            ? 'KoyaBranch'
            : 'FutamataBranch';
    ...
    trainTimetableRefData[key]
      ...
      .forEach(function(table) {
        ...
        tt.forEach(function(obj, i) {
          ...
          var next = tt[i + 1] || {};
          if (
            (obj.as || obj.ds) === 'Toei.Oedo.Tochomae' &&
          ...
          sublines
            ...
            .map(function(subline, i) {
              ...
              function smoothCoords(reverse) {
                ...
                var transition = Math.abs(nextSubline.offset) * 0.5 + 0.5;
                var factors = [];
                var j, distance;
                ...
                for (j = start; j !== end && factors[j] > 0; j += step) {
                  coordinates[j] = turf.getCoord(turf.destination(coordinates[j], baseOffset * factors[j], nearest.bearing));
                }
              }
              ...
              if (subline.type === 'interpolate') {
                ...
                for (j = 1; j < interpolate; j++) {
                  ...
                  coordinates.push([coord1[0] * (1 - f) + coord2[0] * f, coord1[1] * (1 - f) + coord2[1] * f]);
                }
                subline.feature = turf.lineString(coordinates);
              } else if (subline.type === 'main') {
                coordinates = subline.coords.map(function(d) {
                  return d.slice();
                });
  ...
  map.once('styledata', function() {
    ...
    map.addLayer(
      new MapboxLayer({
        ...
        lineWidthUnits: 'pixels',
        lineWidthScale: clamp(Math.pow(2, map.getZoom() - 12), 0.125, 1),
        lineJointRounded: true,
      }),
      'building-3d',
    );
    ...
    map.on('zoom', function() {
      var lineWidthScale = clamp(Math.pow(2, map.getZoom() - 12), 0.125, 1);
...
function lineOffset(geojson, distance) {
  ...
  var polygonLine = turf.polygonToLine(turf.buffer(geojson, dist, { step: coordsLen * 2 + 64 }));
...
function loadRailwayRefData() {
  return loadJSON(
    API_URL +
      'odpt:Railway?odpt:operator=' +
      ['JR-East', 'TWR', 'TokyoMetro', 'Toei', 'Keio']
        .map(function(operator) {
...
function loadTrainTimetableRefData() {
  return loadJSON('data-extra/railways.json').then(function(railwayData) {
    return Promise.all(
      ['Weekday', 'SaturdayHoliday'].map(function(calendar) {
        return Promise.all(
          railwayData.map(function(railway) {
            var id = railway.id;
            return loadJSON(
              API_URL +
                'odpt:TrainTimetable?odpt:railway=odpt.Railway:' +
                id +
