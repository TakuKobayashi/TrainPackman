...
var modelOrigin = mapboxgl.MercatorCoordinate.fromLngLat([139.767, 35.6814]);
...
timetableRefData.forEach(function(train) {
  ...
  train.end = getTime(table[length - 1].dt || table[length - 1].at || table[Math.max(length - 2, 0)].dt);
...
map.once('styledata', function() {
  ...
  function setFlightStandingStatus(flight, standing) {
    ...
    flight.description =
      '<span class="desc-box" style="background-color: ' +
      (operatorLookup[airlineID].tailcolor || '#FFFFFF') +
      ';"></span> ' +
      '<strong>' +
      ...
      '<br><strong>' +
      dict['scheduled-' + (destination ? 'departure' : 'arrival') + '-time'] +
      ':</strong> ' +
      scheduledTime +
